...
> 新 Session 启动时，第一步读取 `.learnings/` 与 `02-方案与评审文档/HANDOFF.md`，瞬间唤醒全部上下文记忆与行为红线。
...
2. **`three-layer-memory` (三层记忆架构)**：
   ...
   - **L3 语义长期记忆 (Semantic Long-term Memory)**：物理机理公式、实验金数据与代码契约（`02-方案与评审文档/HANDOFF.md`）；
...
1. **自动读取记忆核心**：首先阅读 `.learnings/LEARNINGS.md`、`.learnings/ERRORS.md` 与 `02-方案与评审文档/HANDOFF.md`；
